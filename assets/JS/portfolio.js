...
cards.forEach((card) => {
  card.addEventListener("click", function () {
    ...
    if (document.getElementById("modal" + card.id)) {
      ...
      document
        .getElementById("modal" + card.id)
        .classList.add("animationmodalopen");
      document.getElementById("modal" + card.id).style.display = "block";
      document
        .querySelector(
          "#modal" + card.id + ">.modal-content>.modal-header>.close"
        )
        .addEventListener("click", function () {
          document
            .getElementById("modal" + card.id)
            .classList.replace("animationmodalopen", "animationmodalclose");
          ...
          setTimeout(function () {
            ...
            document
              .getElementById("modal" + card.id)
              .classList.remove("animationmodalclose");
          }, 900);
        });
    }
  });
